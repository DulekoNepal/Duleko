...
import { Check, Handshake, MessageSquare, Phone, PhoneOff, ShieldCheck, Users, Globe } from "lucide-react";
...
const OPTIONS: { value: CallPermission; icon: LucideIcon; label: StringKey; hint: StringKey }[] = [
  { value: "everyone", icon: Globe, label: "callEveryone", hint: "callEveryoneHint" },
  { value: "accepted_work", icon: Handshake, label: "callAcceptedWork", hint: "callAcceptedWorkHint" },
  { value: "friends", icon: Users, label: "callFriends", hint: "callFriendsHint" },
  { value: "nobody", icon: PhoneOff, label: "callNobody", hint: "callNobodyHint" },
];
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    selected ? "border-brand-700 bg-brand-50" : "border-transparent bg-slate-50 hover:bg-slate-100",
                  )}
                >
                  <Icon
                    className={cn("mt-0.5 h-4 w-4 shrink-0", selected ? "text-brand-700" : "text-slate-500")}
                    aria-hidden
                  />
                  <span className="min-w-0 flex-1">
                    <span className={cn("block text-sm font-medium", selected ? "text-brand-800" : "text-slate-800")}>
                      {t(label)}
                    </span>
